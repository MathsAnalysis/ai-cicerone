...
// Mappa Leaflet su tile Esri "World Light Gray" (gratuiti, senza chiave; in produzione: provider
// contrattualizzato o self-hosted, es. OpenFreeMap). Nessun servizio Google.
...
export default function TourMap({ routeKey, center, stops, current, done, pos, follow, onPick }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      lf.tileLayer(TILES, { maxZoom: 19, attribution: ATTRIB }).addTo(m);
  ...
  useEffect(() => {
    ...
    shown.forEach((pts, i) => {
      lf.polyline(pts, { color: '#C9C2B8', weight: 3, dashArray: '2 7', opacity: 0.9 }).addTo(g);
  ...
  useEffect(() => {
    ...
    leg(at, stops[current].c).then((pts) => {
      ...
      nav.current?.remove();
      nav.current = lf.polyline(pts, { color: '#B85C38', weight: 4, opacity: 0.85 }).addTo(map.current);
    });
    return () => { alive = false; };
